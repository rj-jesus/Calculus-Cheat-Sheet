...
\subsection{Infinitésimas Equivalentes}
...
Se $\lim\limits_{x \to a} f(x) = 1$ e $\lim\limits_{x \to a} g(x) = \infty$, então\\
$$
\lim\limits_{x \to a} \left[ f(x) \right]^{g(x)} = \lim\limits_{x \to a} \left[ (1 + (f(x) - 1))^{\frac{1}{f(x) - 1}} \right]^{(f(x) - 1)g(x)}$$\\$$
= e ^ {\lim\limits_{x \to a} (f(x) - 1)g(x)}
$$\\
...
\subsection{Derivadas Logarítmicas e Exponenciais}
...
\subsubsection{Integrais Exponenciais/Logarítmicos}
...
Seja $f$ uma função contínua em $[a, b]$ e $x = \varphi(t)$ uma aplicação com derivada contínua e que não anula:
...
Integral de Riemann é o limite da soma de Riemann.\\
Soma de Riemann:\\
...
$\int_a^b f(x)\ dx = ?$ (integral definido)\\
...
\underline{Integral Impróprio Convergente}, caso contrário, \textbf{divergente}.
